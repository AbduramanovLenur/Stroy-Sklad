...
export const getList = (api, page, limit, search, info) => request({ url: `${api}/GetList?page=${page}&limit=${limit}${search ? `&search=${search}` : ''}${info?.isAdmin ? `&isAdmin=${info?.isAdmin}` : ''}${info?.organizationId ? `&OrganizationId=${info?.organizationId}` : ''}${info?.roleId ? `&RoleId=${info?.roleId}` : ''}`, method: "GET" });
